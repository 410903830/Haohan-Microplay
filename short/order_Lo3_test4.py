...
def OrderRangeMKT(Broker,Product,BS, Qty,DayTrade='0',OrderPriceLevel=5,Wait=10):
#def RangeMKT(Broker,Product,BS,Price,Qty,DayTrade,n):
    ...
    if BS == 'B':
        OrderPoint=UpdnInfo[1+OrderPriceLevel*2*2-1]
    # 如果是賣單，則掛下 "OrderPriceLevel" 檔委託
    elif BS == 'S':
        OrderPoint=UpdnInfo[1+OrderPriceLevel*2-1]
    # 送出交易委託
...
def RangeMKTDeal(Broker,Product,BS, Qty,DayTrade='0',OrderPriceLevel=5,Wait=10):
    # 防止例外狀況，最多下三次單
    for i in range(3):
